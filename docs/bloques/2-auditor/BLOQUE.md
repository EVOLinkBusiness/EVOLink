...
- **Entrada:** formulario formal `IntakeForm` (anexo 12/06/2026) — obligatorios (nombre, nicho, zona, teléfono, ¿web?, ¿ficha Maps?) + opcionales. Dos vías lo rellenan: captura de Maps por visión (v1, `extract-presence`) o Places API (v2). El humano confirma antes de auditar; el scoring opera siempre sobre el formulario confirmado, nunca sobre la captura.
