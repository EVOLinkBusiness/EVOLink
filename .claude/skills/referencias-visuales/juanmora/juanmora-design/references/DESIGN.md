...
> **⚠️ CORRECCIÓN EVOLink (override del extractor estático).** El análisis estático
> lee `font-family` computado y reporta `Arial` / `webflow-icons`. Eso es **incorrecto**:
> - **`webflow-icons` NO es una tipografía de texto** — es una *icon font* generada con
>   IcoMoon (su propio base64 dice "Font generated by IcoMoon"). **Nunca** usarla para
>   display ni body.
> - **`Arial` es solo el fallback de sistema.** La fuente real de display/headings es
>   **`Goga`** (`Goga-Regular.otf` 400 · `Goga-Medium.otf` 500 · `Goga-SemiBold.otf` 600),
>   cargada vía `@font-face` desde `https://juanmora.co/fonts/`.
>
> **Mapeo correcto:** Display/Headings → **Goga** · Body/UI → **Arial** (system) ·
> Iconos → webflow-icons (solo glyphs, no texto). Las tablas auto-generadas de abajo
> conservan los valores computados como referencia técnica, pero rigen estos.

**Font Stack:**
- **Goga** — Display / Headings (Goga-Regular/Medium/SemiBold; fuente real de marca)
- **Arial** — Body, Caption, UI (system fallback)
- ~~webflow-icons~~ — icon font (IcoMoon), NO usar como tipografía de texto
